...
function calculateFundingPayout(positionSize, fundingRate, leverage = 10) {
  ...
  fetch: async (req) => {
    ...
    if (url.pathname === "/entrypoints/get-funding/invoke" && req.method === "POST") {
      try {
        ...
        const results = filteredMarkets.map((market) => ({
          ...
          skew: market.skew,
          funding_payout_10x: calculateFundingPayout(1e5, market.funding_rate, 10),
          funding_payout_20x: calculateFundingPayout(1e5, market.funding_rate, 20)
        }));
        return new Response(JSON.stringify({
          output: {
            markets: results,
    ...
    if (url.pathname === "/entrypoints/scan-markets/invoke" && req.method === "POST") {
      try {
        ...
        const highestFunding = filteredMarkets.reduce((max, m) => m.funding_rate > max.funding_rate ? m : max, filteredMarkets[0]);
        return new Response(JSON.stringify({
          output: {
